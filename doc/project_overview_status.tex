...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{UI Layer}: Streamlit frontend (Python) and React frontend (JavaScript) for submission, polling, resume action, and video preview.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item React UI (modern JavaScript-based alternative) with dark theme, responsive design, and Tailwind CSS styling.
\end{enumerate}
...
\section*{UI Options}
\begin{itemize}[leftmargin=*]
  \item \textbf{Streamlit UI} (\texttt{ui/streamlit\_app.py}): Python-based, simple built-in hot reload, minimal setup.
  \item \textbf{React UI} (\texttt{ui/react/}): Modern JavaScript framework, fully customizable styling, Tailwind CSS dark theme, responsive mobile design.
  \begin{itemize}
    \item Setup: \texttt{cd ui/react \&\& npm install \&\& npm start}
    \item Runs at \texttt{http://localhost:3000}
    \item Production build: \texttt{npm run build}
  \end{itemize}
\end{itemize}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item Deploy React UI to CDN or static hosting (Netlify, Vercel, etc.) for standalone frontend.
\end{enumerate}
